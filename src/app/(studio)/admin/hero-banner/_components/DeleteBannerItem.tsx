...
import { deleteHeroBanner } from "@/app/actions/hero-banner";

export default function DeleteBannerItem({ id }: { id: number }) {
  ...
  const handleClick = () => {
    startTransition(async () => {
      try {
        ...
        if (response?.success) {
          const msg = response?.message || "Hero banner deleted successfully";
          successDialog(msg);
        } else {
          toast.error(response?.message || "Failed to delete hero banner");
        }
      } catch (err: any) {
  ...
  return (
    ...
    >
      {isPending ? (
        <span className="w-5 h-5 border-2 border-gray-300 rounded-full hover:bg-blue-light-5 border-blue animate-spin" />
      ) : (
        <TrashIcon />
      )}
    </button>
