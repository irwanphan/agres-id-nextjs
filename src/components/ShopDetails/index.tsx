...
const ShopDetails = ({ product, avgRating, totalRating }: IProps) => {
  ...
  // const handleAddToCart = (isCheckout: boolean = false) => {
  //   if (quantity > product.quantity) {
  //     toast.error(`Only ${product.quantity} available in stock!`);
  //     return;
  //   }
  //   if (isCheckout) {
  //     const isAlreadyItemInCart = Object.values(cartDetails ?? {}).find(
  //       (c) => cartItem.id === product.id
  //     )
  //     if (isAlreadyItemInCart) {
  //       router.push("/checkout");
  //       return;
  ...
  //     // Increment to the desired quantity (quantity - 1 times, since 1 is already added)
  //     for (let i = 1; i < quantity; i++) {
  //       incrementItem(cartItem.id); // Assuming cartItem.id is the unique identifier
  //     }
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {product.discountedPrice &&
                      product.discountedPrice < product.price && (
                        <div className="inline-flex font-medium text-custom-sm text-white bg-blue rounded-full py-0.5 px-2.5">
                          {Math.round(
                  ...
                  <form onSubmit={(e) => e.preventDefault()}>
                    ...
                    <div className="flex flex-wrap items-center gap-4.5">
                      ...
                      <button
                        ...
                        className={`inline-flex font-medium text-white bg-dark py-3 px-7 rounded-lg ease-out duration-200 hover:bg-dark-2 ${isAlradyAdded && "cursor-not-allowed bg-dark-2"
                          }`}
                      >
                        {isAlradyAdded
                          ? "Added"
                      ...
                      <button
                        ...
                        className={`flex items-center justify-center w-12 h-12 duration-200 ease-out border rounded-lg border-gray-3 hover:text-white hover:bg-dark hover:border-transparent ${isAlreadyWishListed
