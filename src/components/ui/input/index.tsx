...
export function InputGroup({
  ...
  const id = useId();
  return (
    <div className={`${props.type === "checkbox" || props.type === "radio" ? "flex flex-row-reverse justify-end items-center gap-2" : ""}`}>
      <label
        htmlFor={id}
        className={`
          block text-sm font-normal text-gray-6
          ${props.type === "checkbox" || props.type === "radio" ? "mb-0" : "mb-1.5"}
        `}
      >
      ...
      <input
        ...
        className={cn(`
          ...
          ${props.type === "checkbox" || props.type === "radio" ? "w-5 h-5 p-2 rounded-sm" : "w-full h-11 rounded-lg"}
          ${props.readOnly ? "bg-gray-2" : ""}
        `,
          className,
          {
            "border-red": error,
